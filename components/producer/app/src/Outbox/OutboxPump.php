 ...
 *
 * BEAR.Sunday CLI コンテキストで動作する独立プロセス。
...
class OutboxPump
{
    ...
    public function run(): void
    {
        ...
        Coroutine::create(function () use ($channel) {
            while (true) {
                try {
                    $this->subscriber->subscribe([OutboxChannel::NOTIFY], function (\Redis $redis, string $ch, string $msg) use ($channel) {
                        $channel->push(true);
                        $redis->unsubscribe([OutboxChannel::NOTIFY]);
                    });
